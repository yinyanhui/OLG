...
D = .3; % fraction of capital that is lost
...
% Those who where alive before the shock hold a fraction D of the amount of assets 
% corresponding to the steady state
x0 = fsolve(@(x) assets(x,rss*ones(J,1),wss*ones(J,1),0,beta,gamma,J),0.1*ones(J-1,1)); 
a0 = (1-D)*[x0',0];

% Since supply of capital contracted, we guess the interest rate goes up 
%to k that solve the firms FOC and then converges to SS value linearly
rssD = (alpha)*z*(kss*(1-D))^(1-alpha)-delta;
rr0=[rssD; zeros(T+J-1,1)];
for t=1:T
    rr0(t)=rss*(t/T)+(T-t)/(T)*rssD;
end
for t=T+1:T+J-1
    rr0(t)=rss;
end
...
% These guys are born after the shock
for t=1:T
...
% These guys had age i when hit by the shock
for i = 2:J-1
